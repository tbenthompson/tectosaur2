...
Before we get into the problem, I'll rebuild the same simple (boring?) surface self-interaction matrix from previous sections. Our initial goal will be to approximate this matrix.  
...
full_mat = cutde.disp_matrix(surf_centroids + np.array([0,0,0.01]), surf_tri_pts, 0.25)
tmp = full_mat[:,:,:,0].copy()
...
def build_tree_node(all_pts, all_radii, min_pts_per_box, ordered_idxs, idx_start, idx_end):
    ...
    node = TreeNode(
        idx_start, idx_end, box_center, box_radius,
    ...
    # problem.
    node.left = build_tree_node(all_pts, all_radii, min_pts_per_box, ordered_idxs, idx_start, idx_split)
    node.right = build_tree_node(all_pts, all_radii, min_pts_per_box, ordered_idxs, idx_split, idx_end)
...
def check_tree(pts, radii, tree, node):
    ...
    if np.any(dist > node.radius):
        ...
        return (
            check_tree(pts, radii, tree, node.left) and
            check_tree(pts, radii, tree, node.right)
        )
...
for depth in range(9):
    ...
    plt.xlim([tree.root.center[0] - tree.root.radius, tree.root.center[0] + tree.root.radius])
    plt.ylim([tree.root.center[1] - tree.root.radius, tree.root.center[1] + tree.root.radius])
plt.tight_layout()
plt.show()
```
...
\begin{equation}
\mathrm{farfield~if~ ~} \|c_2 - c_1\| < S (r_1 + r_2)
\end{equation}
    ...
    for target_obsn in path_nodes:
        for obsn, srcn in direct:
            if obsn is target_obsn:
                circle = plt.Circle(tuple(srcn.center[:2]), srcn.radius, fill=False, color='r')
                plt.gca().add_patch(circle)

    for target_obsn in path_nodes:
        for obsn, srcn in approx:
            if obsn is target_obsn:
                circle = plt.Circle(tuple(srcn.center[:2]), srcn.radius, fill=False, color='b')
                plt.gca().add_patch(circle)        
    circle = plt.Circle(tuple(target_obsn.center[:2]), target_obsn.radius, fill=True, color='k')
    plt.gca().add_patch(circle)
    plt.xlabel('$x$')
    plt.ylabel('$y$')
    plt.xlim([tree.root.center[0] - tree.root.radius, tree.root.center[0] + tree.root.radius])
...
```python
%%time
tree_tri_pts = surf_tri_pts[tree.ordered_idxs]
tree_obs_pts = np.mean(tree_tri_pts, axis=1) + np.array([0,0,0.01])
...
packed_blocks, block_starts = cutde.disp_block(
    tree_obs_pts, tree_tri_pts, 
    direct_obs_starts, direct_obs_ends, direct_src_starts, direct_src_ends, 
    0.25
)
for i in range(block_starts.shape[0] - 1):
    ...
    DD = packed_blocks[block_starts[i]:block_starts[i+1]].reshape(
        (
            direct_obs_ends[i] - direct_obs_starts[i], 3, 
             direct_src_ends[i] - direct_src_starts[i], 3
        )
    )
    tmp = DD[:,:,:,0].copy()
    DD[:,:,:,0] = DD[:,:,:,1]
    DD[:,:,:,1] = tmp
    packed_blocks[block_starts[i]:block_starts[i+1]] = DD.flatten()
```
...
The `cutde.disp_blocks` version is about 7x faster. Below, we quickly check that both these implementations provide almost exactly the same output (down to `1e-15`), providing a bit more confidence that either one is correct.

```python
packed_err = [
    np.max(direct_blocks[i].flatten() - packed_blocks[block_starts[i]:block_starts[i+1]])
...
The (CITE BRADLEY AND ALSO GIVE A LINK) bradley paper works through the details of this approach. 
...
sample = cutde.disp_matrix(surf_centroids[row_idxs] + np.array([0,0,0.01]), surf_tri_pts, 0.25)
sample.shape
```

Then, with [LINK TO WIKIPEDIA!! jackknife resampling](a), we can also recover a standard deviation for the norm estimate. 

```python
n_full_entries = ((surf_centroids.shape[0] * 3) ** 2)
frob2_ests = [np.sum(sample[i] ** 2) * (n_full_entries / sample[i].size) for i in range(n_samples)]
...
jack_stddev = np.sqrt(np.sum((jack_replicates - jack_mean) ** 2) * ((n_samples - 1) / n_samples))
...
Finally, we can compute the tolerance for each block and plot a histogram of the results.

```python
n_entries_per_block = np.array([
    (obs_node.idx_end - obs_node.idx_start) * 3 
    * (src_node.idx_end - src_node.idx_start) * 3
...
Given the previous approximation code and the block-wise error tolerance, you'd think we're ready to go for building the entire approximate portion of the H-matrix. And that's true. But, even for a fairly small problem like the one we're dealing with here, the Python implementation of the `ACA_plus` function is quite slow and would take several minutes to build all 4000+ approximate blocks. So, instead, I implemented a GPU-optimized version in `cutde` with the `cutde.disp_aca` function. 
...
And since, if you haven't noticed already, I really like checking things, let's look at the Frobenius norm error between these approximation and the exact matrix block. These are all below the requested tolerances.
...
for i in range(10):
    ...
    if i == 0:
        print(f'All good! Error less than tolerance: {error:.3} < {block_tolerances[i]:.3}')
```
...
The SVD recompression is not optimized beyond the version from before, but it requires much less time. It would be possible to reduce the runtime of that portion by 5-20x using a parallel or GPU-optimized version. However, it's not currently slow enough to justify that. 
...
approx_blocks_gpu_aca = cutde.disp_aca(
    tree_obs_pts, tree_tri_pts, 
    approx_obs_starts, approx_obs_ends, 
    approx_src_starts, approx_src_ends,
    0.25, block_tolerances / 50.0, max_iter
)
...
Great! Despite working with over 40x more blocks, this implementation ran faster. 

And a quick check to make sure the results match for the first ten blocks:
...
for i in range(10):
    ...
    assert(np.sqrt(np.sum((est1 - est2) ** 2)) < block_tolerances[i])
    assert(np.sqrt(np.sum((est2 - truth) ** 2)) < block_tolerances[i])
```
...
```cython tags=[] magic_args="--compile-args=-fopenmp --link-args=-fopenmp --verbose"
import numpy as np
...
cdef extern from *:
    """
    void omp_atomic_add(double* x, double y) {
        #pragma omp atomic
        *x += y;
    }
    """
    void omp_atomic_add(double* x, double y) nogil

@cython.boundscheck(False)
@cython.wraparound(False)
cdef void single_block_dot(long n_rows, long n_cols,  
                           double* packed_blocks, double* x_tree, 
                           double* y_tree) nogil:
    cdef int row_start
    cdef int i, j
    cdef double out_v
    for i in range(n_rows):
        ...
        for j in range(n_cols):
            out_v += packed_blocks[row_start + j] * x_tree[j]
...
def approx_dot(long[::1] obs_start, long[::1] obs_end, 
    ...
    cdef int max_rank = np.max([a[0].shape[1] for a in approx_UV])
    cdef temp_buffer = np.empty(n_threads * max_rank)
...
A quick performance note: This comparison is mildly unfair to the H-matrix algorithm. The BLAS implementation behind the numpy matrix-vector product has been optimized to the hilt. It's probably either OpenBLAS or MKL depending on your setup and both of them should be achieving something near the peak possible performance for a dense matrix-vector. On the other hand, the  Cython code above is not optimized to that level. Many improvements are possible -- explicit vectorization (AVX, AVX-512), loop unrolling, removing the atomic operations, etc, etc, etc. There's probably another factor of 2-3x on the table. So, the fact that, despite this disadvantage, the H-matrix implementation is several times faster than the dense matrix-vector product is super exciting! 
...
* Performance optimization the SVD recompression.
* There are many other $O(n)$ sparsification methods for these types of problems. 
  ...
  * $\mathcal{H}^2$ methods are an extension of the H-matrix techniques here and further improve both memory usage and runtime.
...
* It's possible to perform efficient LU decomposition of an H-matrix. That would be super useful for problems where we're solving many problems using the same left hand side matrix (e.g. time dependent problem). 
* And many more!
