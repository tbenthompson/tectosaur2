...
struct NearfieldObsInfo {
    double x;
    double y;
    double* parameters;
};

inline std::array<double, 1> single_layer(const NearfieldObsInfo& obs, double srcx,
...
inline std::array<double, 1> double_layer(const NearfieldObsInfo& obs, double srcx,
                                          double srcy, double srcnx, double srcny) {
    double dx = obs.x - srcx;
    double dy = obs.y - srcy;
    double r2 = dx * dx + dy * dy;
...
inline std::array<double, 2> adjoint_double_layer(const NearfieldObsInfo& obs,
...
inline std::array<double, 2> hypersingular(const NearfieldObsInfo& obs, double srcx,
                                           double srcy, double srcnx, double srcny) {
    double dx = obs.x - srcx;
    double dy = obs.y - srcy;
    double r2 = dx * dx + dy * dy;
...
inline std::array<double, 4> elastic_U(const NearfieldObsInfo& obs, double srcx,
                                       double srcy, double srcnx, double srcny) {
...
inline std::array<double, 4> elastic_T(const NearfieldObsInfo& obs, double srcx,
                                       double srcy, double srcnx, double srcny) {
    double poisson_ratio = obs.parameters[0];
...
template <typename K> void _nearfield_integrals(K kernel_fnc, const NearfieldArgs& a) {
    ...
    constexpr size_t ndim =
        std::tuple_size<decltype(kernel_fnc(NearfieldObsInfo{}, 0, 0, 0, 0))>::value;
...
#pragma omp parallel
    {
        ...
        for (int integral_idx = start; integral_idx < end; integral_idx++) {
            ...
            NearfieldObsInfo obs{a.obs_pts[obs_i * 2 + 0], a.obs_pts[obs_i * 2 + 1],
                                 a.kernel_parameters};
