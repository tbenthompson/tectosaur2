...
2. The error is extremely low everywhere else. We are seeing 6 to 10 digits of accuracy!
3. The error is highest near the fault, almost 100x higher than further from the fault. The discretization error is likely to be highest where the gradients in the solution are largest. The gradients are largest near the fault. This could be remedied by having a denser boundary grid near the fault.
...
sym_t = sp.symbols("t")
sym_x = -surf_L * sym_t
sym_y = sp.exp(-(sym_t ** 2) * 200) * sp.Rational(2.0) - sp.Rational(2.0)
