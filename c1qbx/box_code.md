...
```{code-cell} ipython3
new_tree = build_box_tree(ethridge_laplacian_fnc, np.array([[0, 0]]), np.array([1]), 10, 0.0000000001)
print(f'built tree with {new_tree.leaves.centers.shape[0]} leaves')
...
```{code-cell} ipython3
new_tree = build_box_tree(ethridge_laplacian_fnc, np.array([[0, 0]]), np.array([1]), 10, 0.000001)
print(f'built tree with {new_tree.leaves.centers.shape[0]} leaves')
#balance_21(new_tree, ethridge_laplacian_fnc)
#new_tree = calculate_leaves(new_tree)
print(f'balanced tree has {new_tree.leaves.centers.shape[0]} leaves')
...
Error stalls out at `1.487e-06`. Potential reasons why:
1. The tree construction is broken. **Check that the tree is integrating some simple function very exactly.**
2. The point at which I am evaluating doesn't require any balancing, so I can turn that off to remove that potential cause of error.
3. The most likely is probably that the nearfield integrals are broken somehow. **I could test this by analytically computing a few of the nearfield integrals and comparing.**
4. The interpolation is probably not an issue because the cell errors are also unexpectedly high. 
5. The laplacian or the solution functions could be slightly wrong somehow. 
